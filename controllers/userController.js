...
export const register = catchAsyncError(async (req, res, next) => {
    ...
    const file = req.file;
    if (!name || !email || !password || !file) return next(new ErrorHandler("Please add all fields", 400));
...
export const login = catchAsyncError(async (req, res, next) => {
    ...
    if (!email || !password) return next(new ErrorHandler("Please add all fields", 400));
...
export const forgetPassword = catchAsyncError(async (req, res, next) => {
    ...
    await user.save();
    //http://localhost:3000/resetpassword/somerandomtoken
